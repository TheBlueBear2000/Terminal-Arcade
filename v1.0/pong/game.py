...
def printBoard(paddles, ball, paused = False):
    ...
    for y in range(HEIGHT):
        ...
        if y == HEIGHT//2 and paused:
            for x in range((WIDTH*2 - (len(pauseMsg)))//4 + 1):
                frame += makeChar(paddles, ball, x , y)
            frame += pauseMsg
            for x in range((WIDTH*2 - (len(pauseMsg)))//4):
                frame += makeChar(paddles, ball, x + ((WIDTH*2 + (len(pauseMsg)))//4 + 1) , y)
        else:
            for x in range(WIDTH):
                frame += makeChar(paddles, ball, x, y)
    ...
    if multiplayer:
        frame += centerTo(f"Player 1 score: {paddles[0].score}", WIDTH + 1) + centerTo(f"Player 2 score: {paddles[1].score}", WIDTH + 1)
    else:
        frame += centerTo(f"Score: {paddles[0].score}", WIDTH*2+2)
...
def printMenu(winner, keypos, multiplayer, baseMultiplayer, rally, scores):
    ...
    if winner == 0:
        frame += "|" + centerTo("Well played!", WIDTH*2) + "|\n"
        frame += "|" + centerTo(f"You got {scores[0]} points!", WIDTH*2) + "|\n"
    elif winner == 3:
        frame += "|" + centerTo("", WIDTH*2) + "|\n"
        if baseMultiplayer:
            frame += "|" + centerTo(f"Player 1: {scores[0]}  Player 2: {scores[1]}", WIDTH*2) + "|\n"
        else:
            frame += "|" + centerTo("", WIDTH*2) + "|\n"
    elif winner == 4:
        frame += "|" + centerTo("", WIDTH*2) + "|\n"
        frame += "|" + centerTo(f"Welcome to...", WIDTH*2) + "|\n"
    else:
        frame += "|" + centerTo(f"Player {winner} wins!", WIDTH*2) + "|\n"
        frame += "|" + centerTo("", WIDTH*2) + "|\n"
        
    frame += "|" + centerTo("", WIDTH*2) + "|\n"
    
    for row in deathIcon:
        frame += "|" + centerTo(row, WIDTH*2) + "|\n"
    
    frame += "|" + centerTo("", WIDTH*2) + "|\n"
    if keypos == 0:
        frame += "|" + centerTo("> Play  ", WIDTH*2) + "|\n"
    else:
        frame += "|" + centerTo("  Play  ", WIDTH*2) + "|\n"
